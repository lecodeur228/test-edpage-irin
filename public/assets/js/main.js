...
//========== PRELOADER ============= //
function hideVoltzPreloader() {
  $(".preloader").each(function () {
    $(this).addClass("is-hidden").stop(true, true).fadeOut(200);
  });
}
function watchVoltzPreloader() {
  if ($(".preloader").length) {
    hideVoltzPreloader();
    return;
  }
  ...
  var timer = setInterval(function () {
    attempts += 1;
    if ($(".preloader").length || attempts > 50) {
      clearInterval(timer);
      hideVoltzPreloader();
    }
  }, 100);
...
$(window).on("load", function () {
  setTimeout(watchVoltzPreloader, 200);
});
watchVoltzPreloader();
